...
def get_total_booked_count(p_tour_id):
    """Calculates the total number of participants booked for a specific tour."""
...
def save_tour_report(p_tour_id, p_actual_count, p_photo_path):
    ...
    conn = sqlite3.connect("roma_tours.db")
    cursor = conn.cursor()
    cursor.execute(query, (p_actual_count, p_photo_path, p_tour_id)) # Note: Corrected parameter order to match query
    conn.commit()
    cursor.close()
    conn.close()